...
# Pools
unauthenticated: Dict[str, WebSocket] = {}
...
async def health(request: Request):
    ...
    print(f"[HEALTH] Ping from {client_ip}")
    return {"status": "ok"}
...
async def receive_command(request: Request):
    ...
    target = data.get("deviceName") or data.get("deviceId")
    print(f"[COMMAND] Received: {data} from {request.client.host}")
...
async def send_command_to_device(deviceName: str, command: str):
    ...
    if ws:
        try:
            ...
            print(f"[SEND] Error sending to {deviceName}: {e}")
            traceback.print_exc()
...
def notify_server_a(deviceName: str, status: str, payload: dict = None):
    ...
    try:
        ...
        print(f"[NOTIFY] Failed to notify Server A: {e}")

@app.websocket("/ws/device")
async def ws_device(websocket: WebSocket, deviceId: str = Query(...)):
    await websocket.accept()
    client_ip = websocket.client.host
    print(f"[WS] New connection from {client_ip}, deviceId={deviceId}")
    ...
    try:
        while True:
            ...
            print(f"[WS] Raw message from {deviceId}: {msg}")
            data = json.loads(msg)
            ...
            if data.get("type") == "auth":
                ...
                try:
                    ...
                    print(f"[AUTH] Backend replied HTTP {resp.status_code}")
                    print(f"[AUTH] Headers: {resp.headers}")
                    print(f"[AUTH] Body: {resp.text}")

                    if resp.status_code == 200:
                        backend = resp.json()
                        print(f"[AUTH] Parsed JSON: {backend}")
                        ...
                        if backend.get("success"):
                            ...
                            print(f"[AUTH] Device {deviceName} authenticated")
                            ...
                            print(f"[WS] Closing socket for {deviceId} with code 4002, reason: {reason}")
                            await websocket.close(code=4002)
                            notify_server_a(deviceId, "REJECTED", {"reason": reason})
                            return
                    else:
                        reason = f"Backend error HTTP {resp.status_code}"
                        print(f"[AUTH] Error: {reason}")
                        await websocket.send_text(json.dumps({"type": "auth_failed", "reason": reason}))
                        print(f"[WS] Closing socket for {deviceId} with code 4001, reason: {reason}")
                        await websocket.close(code=4001)
                        notify_server_a(deviceId, "REJECTED", {"reason": reason})
                        return
                except Exception as e:
                    reason = f"Internal error: {e}"
                    print(f"[AUTH] Exception: {reason}")
                    traceback.print_exc()
                    await websocket.send_text(json.dumps({"type": "auth_failed", "reason": reason}))
                    print(f"[WS] Closing socket for {deviceId} with code 1011, reason: {reason}")
                ...
                last_ping[deviceName] = time.time()
                print(f"[PING] Received ping from {deviceName}, sending pong")
                await websocket.send_text(json.dumps({"type": "pong"}))

            elif deviceName in devices and data.get("type") == "status":
                print(f"[STATUS] Forwarding status from {deviceName}: {data}")
                notify_server_a(deviceName, "STATUS", payload=data)

            if deviceId in unauthenticated and time.time() > auth_deadline:
                reason = "Timeout: no auth within 60s"
                print(f"[AUTH] Timeout for {deviceId}")
                await websocket.send_text(json.dumps({"type": "auth_failed", "reason": reason}))
                print(f"[WS] Closing socket for {deviceId} with code 4003, reason: {reason}")
        ...
        print(f"[WS] Device {deviceId} disconnected")
        unauthenticated.pop(deviceId, None)
        if deviceName and deviceName in devices:
            devices.pop(deviceName, None)
            last_ping.pop(deviceName, None)
            print(f"[WS] Device {deviceName} removed from pool")
        ...
        while True:
            ...
            for deviceName in list(last_ping.keys()):
                if now - last_ping[deviceName] > PING_TIMEOUT:
                    print(f"[TIMEOUT] Device {deviceName} timed out (no ping)")
